...
def buy():
    ...
    if request.method == "POST":
        ...
        if request.form.get("shares").isnumeric() == False:
            return apology("symbol must be numeric")
        #minus check
        if (request.form.get("shares")) <= "0":
            return apology("shares must be over zero")
        #cash check (How much do you have?)
        try:
            cash = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
        except:
            return apology("Error db exception")
        #enough chec k(Is the price much expensive than you have?)
        getPrice = lookup(request.form.get("symbol"))['price']
        #buy
        if getPrice < cash[0]["cash"]:
